...
export const downlightTemplate: QuoteTemplateConfig = {
  category: "筒灯",
  ...
  columns: [
    ...
    { header: "Cutout (mm)", key: "cutout", width: 14 },
  ...
  writeHeader: (ws) => writeHeader(ws, downlightTemplate),
  buildRowCells: (item, index) => buildRowCellsFromValues(downlightTemplate.columns, [
      index + 1,
      item.modelNo ?? item.productName,
      appendSuffix(readParam(item, "watts"), "W"),
      readSize(item),
      stripTrailingUnit(readParam(item, "cutout_mm"), "mm"),
