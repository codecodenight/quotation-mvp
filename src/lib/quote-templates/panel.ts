...
export const panelTemplate: QuoteTemplateConfig = {
  ...
  writeHeader: (ws) => writeHeader(ws, panelTemplate),
  buildRowCells: (item, index) => buildRowCellsFromValues(panelTemplate.columns, [
      index + 1,
...
function readPanelSize(item: QuoteTemplateItem): string {
  const explicit = readParam(item, "size_display");
  if (explicit) {
    return stripTrailingUnit(explicit, "mm");
  }

  const length = readParam(item, "length_mm");
  const width = readParam(item, "width_mm");
  const height = readParam(item, "height_mm");
