...
export const magneticTrackTemplate: QuoteTemplateConfig = {
  category: "磁吸灯",
  sheetName: "LED Magnetic Track Light",
  columns: [
    { header: "No.", key: "no", width: 8 },
    { header: "Model No.", key: "modelNo", width: 18 },
    { header: "Power", key: "power", width: 12 },
    { header: "Track System", key: "trackSystem", width: 16 },
  ...
  writeHeader: (ws) => writeHeader(ws, magneticTrackTemplate),
  buildRowCells: (item, index) => buildRowCellsFromValues(magneticTrackTemplate.columns, [
      index + 1,
      item.modelNo ?? item.productName,
      appendSuffix(readParam(item, "watts"), "W"),
      readParam(item, "track_system"),
